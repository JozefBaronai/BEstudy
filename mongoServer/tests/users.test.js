...
describe("registering user", () => {
  beforeEach(async () => {
    ...
    const user = new User({ ...initialUser, password: userHash });
    await user.save();
  });
...
describe("edit user", () => {
  ...
  test("successfully editing user's name", async () => {
    ...
    const result = await api
      ...
      .expect(201)
      .expect("Content-Type", /application\/json/);
    const endingUser = await User.findById(initialUser.id);
  ...
  test("successfully editing user's password", async () => {
    ...
    const userResult = await User.findById(initialUser.id);
